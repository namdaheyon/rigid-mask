...
import time
import cv2
from rigid_mask_ros import geometry as kornia
...
class flow_reg(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.truncated:
            ...
            mask = torch.zeros(
                b, u * v, h, w, device=x.device, dtype=x.dtype)
        ...
        if torch.isnan(x).any():
            # Fall back to a full finite softmax if truncated logits became
            # degenerate. Avoids the deprecated Tensor-to-NumPy conversion.
            finite_logits = torch.nan_to_num(
                oldx.view(b, -1, h, w), nan=0.0, posinf=1e4, neginf=-1e4)
            x = F.softmax(finite_logits, dim=1).view(b, u, v, h, w)
...
def get_grid(B,H,W,device=None):
    ...
    grid = torch.tensor(
        np.concatenate((basex.reshape((-1,H,W,1)),basey.reshape((-1,H,W,1))),-1),
...
class VCN(nn.Module):
    ...
    def diagnostic_tensor(self, name, value):
        ...
        step = max(1, (flat.numel() + 1023) // 1024)
        sample = flat[::step].float().cpu().numpy()
        finite = np.isfinite(sample)
        valid = sample[finite]
        self.stage_diagnostics[name] = dict(
            shape=list(value.shape), sampled=int(sample.size),
            finite_fraction=float(finite.mean()) if sample.size else None,
            p05_p50_p95=np.percentile(valid, [5, 50, 95]).tolist() if valid.size else None)

    def __init__(self, size, md=[4,4,4,4,4], fac=1., exp_unc=True,
                 pose_ransac_threshold=0.0001, foreground_only=False):
        super(VCN,self).__init__()
        self.md = md
        self.fac = fac
        self.pose_ransac_threshold = pose_ransac_threshold
        ...
        if not self.foreground_only:
            ...
            self.det = create_model(
                'dla_34', {'hm': 2, 'wh': 36}, 256, num_input=14)
    ...
    def corrf(self, refimg_fea, targetimg_fea,maxdisp, fac=1):
        if self.training:
            ...
            if refimg_fea.is_cuda:
                cost = torch.zeros(
                    b, c, 2 * maxdisp + 1, 2 * int(maxdisp // fac) + 1,
                    height, width, device=refimg_fea.device,
    ...
    def cost_matching(self,up_flow, c1, c2, flowh, enth, level):
        ...
        if level == 0:
            ...
            flowh = torch.cat((flowhh, F.interpolate(flowh.detach()*2, [flowhh.shape[2],flowhh.shape[3]], mode='bilinear')),1) # b, k2--k2, h, w
            enth = torch.cat((enthh, F.interpolate(enth, [flowhh.shape[2],flowhh.shape[3]], mode='bilinear')),1)
    ...
    def forward(self,im,disc_aux=None,disp_input=None):
        ...
        if not self.training or disc_aux[-1]:
            b,_,h,w = flow2.shape 
            exp2,err2,_ = self.affine(get_grid(b,h,w,im.device)[:,0].permute(0,3,1,2).repeat(b,1,1,1).clone(), flow2.detach(),pw=1)
            x = torch.cat((
            ...
            # depth change net
            iexp2 = F.interpolate(dchange2.clone(), [im.size()[2],im.size()[3]], mode='bilinear')
            ...
            dchange2 = F.interpolate(dchange2, [im.size()[2],im.size()[3]], mode='bilinear')

            if dcneto.shape[1]>1:
                dc_unc = dcneto[:,1:2]
            else:
                dc_unc = torch.zeros_like(dcneto)
            dc_unc = F.interpolate(dc_unc, [im.size()[2],im.size()[3]], mode='bilinear')[:,0]
        ...
        if not self.training or disc_aux[-1]==2:
            ...
            H,W = im.size()[2:4]
            flow = 4*F.interpolate(flow2, [H,W], mode='bilinear').detach()
            oor2 = F.interpolate(oor2[:,np.newaxis], [H,W], mode='bilinear').detach()[:,0]
            ...
            hp0o = torch.stack(torch.meshgrid(
                torch.arange(H, device=im.device, dtype=im.dtype),
                torch.arange(W, device=im.device, dtype=im.dtype),
                indexing='ij'), dim=0).flip(0).unsqueeze(0)
            ...
            hp0d = torch.cat((hp0d, hp0d.new_ones(1,hp0d.shape[1],1)),-1)
            hp1d = torch.cat((hp1d, hp1d.new_ones(bs,hp0d.shape[1],1)),-1)
            ...
            hp0 = torch.cat((hp0, hp0.new_ones(1,hp0.shape[1],1)),-1)
            hp1 = torch.cat((hp1, hp1.new_ones(bs,hp0.shape[1],1)),-1)
            ...
            hp0x = torch.cat((hp0x, hp0x.new_ones(1,hp0x.shape[1],1)),-1)
            hp1x = torch.cat((hp1x, hp1x.new_ones(bs,hp0x.shape[1],1)),-1)
            ...
            if self.training:
            ...
            elif getattr(self, 'replay_pose', None) is not None:
                # Offline full-network A/B: recompute ALL downstream features,
                # learned weights and residual, not just the final weighted sum.
                rot = self.replay_pose['rotation'].to(im.device)
                trans = self.replay_pose['translation'].to(im.device)
                Ex = self.replay_pose['essential']
            else:
                ...
                rotx,transx,Ex = F_ngransac(
                    hp0x, hp1x, Kinv.inverse(), rand, unc_occ,
                    Kn=Kinv_n.inverse(), cv=True,
                    threshold=self.pose_ransac_threshold)
                rot = rotx.to(im.device).detach()
                trans = transx.to(im.device).detach()
            ...
            if disp_input is None:
                with torch.no_grad():
                    self.midas.eval()
                    mean = im.new_tensor([0.485, 0.456, 0.406])[None,:,None,None]
                    std = im.new_tensor([0.229, 0.224, 0.225])[None,:,None,None]
                    input_im = (disc_aux[4].permute(0,3,1,2) - mean) / std
            ...
            self.diagnostic_tensor('midas_inverse_depth_relative', disp)
            self.diagnostic_tensor('depth_contrast_log_ratio', log_dratio)
            self.diagnostic_tensor('epipolar_cost', mcost1)
            self.diagnostic_tensor('angular_3d_cost', mcost4)
            #pdb.set_trace()
            ...
            if getattr(self, 'capture_spatial_diagnostics', False):
                ...
                for name, value in dict(
                        ...
                        geometry_hp1=hp1, geometry_tau=tau,
                        geometry_Kinv=Kinv, geometry_Kinv_next=Kinv_n,
                        geometry_rot=rot, geometry_trans=trans,
                        geometry_essential=Ex).items():
                    self.diagnostic_tensor(name, value)
            fgmask = F.interpolate(fgmask, [im.size()[2],im.size()[3]], mode='bilinear')
            self.diagnostic_tensor('foreground_logits', fgmask)

            # The ROS fusion uses LiDAR clustering for object instances and
            # only needs RigidMask's foreground logits. Skip the later DLA/DCN
            # instance branch to reduce latency and GPU use.
            if not self.training and self.foreground_only:
                return fgmask[0, 0]
...
#                gt_p13d = pts[:,:,gt_p03d.shape[-1]*gt_p03d.shape[-2]:].view(bs,3,gt_p03d.shape[2],gt_p03d.shape[3])
                ...
                raise RuntimeError("The vendored RigidMask port supports inference only")
...
#                loss += loss_pose*1e-6
       ...
       (100*F.interpolate(log_dratio,[H,W], mode='bilinear')[:,0].exp()).clamp(0,255),
            ...
            flow2 = F.interpolate(flow2.detach(), [im.size()[2],im.size()[3]], mode='bilinear')*4
