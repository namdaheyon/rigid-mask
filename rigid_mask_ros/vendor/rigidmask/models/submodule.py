...
def compute_geo_costs(rot, trans, Ex, Kinv, hp0, hp1, tau, Kinv_n=None):
    ...
    mcost1 = sampson_err(Kinv.matmul(hp0.permute(0,2,1)),
                         Kinv_n.matmul(hp1.permute(0,2,1)),Ex.to(hp0.device).permute(0,2,1))  # variable K
...
def get_intrinsics(intr, noise=False):
    ...
    Kinv = torch.eye(3, device=f.device, dtype=f.dtype)[None].repeat(bs,1,1)
    Kinv[:,0,0] =  f/dfx
    Kinv[:,1,1] =  f/dfy
    Kinv[:,0,2] = cx/dfx
    Kinv[:,1,2] = cy/dfy
    ...
    if len(intr)>12:
        Kinv_n = torch.eye(3, device=f.device, dtype=f.dtype)[None].repeat(bs,1,1)
        fn = intr[12].float()
        #Kinv_n[:,2,2] *= fn
        #Kinv_n[:,0,2] -= cx
        #Kinv_n[:,1,2] -= cy
...
def F_ngransac(hp0,hp1,Ks,rand, unc_occ, iters=1000,cv=True,Kn=None,
               threshold=0.0001):
    if Kn is None:
        Kn = Ks
    import cv2
    ...
    if not cv:
        raise RuntimeError("NG-RANSAC is disabled; use the OpenCV pose estimator")

    Es = torch.zeros((b, 3, 3), dtype=torch.float32)
    rot = torch.zeros((b, 3), dtype=torch.float32)
    trans = torch.zeros((b, 3), dtype=torch.float32)

    for i in range(b):
        points1 = np.asarray(hp0[i, :, :2].detach().cpu(), dtype=np.float64)
        points2 = np.asarray(hp1[i, :, :2].detach().cpu(), dtype=np.float64)
        valid = np.isfinite(points1).all(1) & np.isfinite(points2).all(1)
        if unc_occ is not None:
            # oor2 is trained with positive labels for occluded/out-of-range
            # flow. Do not use those correspondences to estimate ego motion.
            uncertainty = np.asarray(unc_occ[i].detach().cpu()).reshape(-1)
            valid &= np.isfinite(uncertainty) & (uncertainty <= 0.0)
        if rand:
            valid &= np.random.random(len(valid)) < 0.1
        points1, points2 = points1[valid], points2[valid]
        if len(points1) < 16:
            raise RuntimeError("OpenCV pose unavailable: only {} reliable matches".format(
                len(points1)))
        E, inlier_mask = cv2.findEssentialMat(
            points1, points2, np.eye(3), cv2.RANSAC, 0.999, threshold,
            maxIters=iters)
        if E is None or E.shape[0] < 3 or not np.isfinite(E).all():
            raise RuntimeError(
                "OpenCV pose estimation is degenerate: no essential matrix")
        else:
            E = E[:3, :3]
            try:
                inliers = int(np.count_nonzero(inlier_mask))
                # The default 50-baseline triangulation limit rejects valid
                # distant road points at low ego speeds. Validate actual
                # rotation-compensated parallax below instead.
                recovered, rotation_10, translation_10, pose_mask, _ = cv2.recoverPose(
                    E, points1, points2, np.eye(3), distanceThresh=10000.0,
                    mask=inlier_mask.copy())
                R01 = rotation_10.T
                t10 = -R01.dot(translation_10[:, 0])
            except cv2.error:
                raise RuntimeError(
                    "OpenCV pose recovery failed for a degenerate frame pair")
            rays0 = np.column_stack((points1, np.ones(len(points1))))
            rays1 = np.column_stack((points2, np.ones(len(points2)))).dot(R01.T)
            rays0 /= np.linalg.norm(rays0, axis=1, keepdims=True)
            rays1 /= np.linalg.norm(rays1, axis=1, keepdims=True)
            angles = np.arctan2(np.linalg.norm(np.cross(rays0, rays1), axis=1),
                                np.sum(rays0 * rays1, axis=1))
            pose_valid = pose_mask.reshape(-1) != 0
            parallax = float(np.median(angles[pose_valid])) if recovered else 0.0
            F_ngransac.last_diagnostics = dict(
                matches=len(points1), inliers=inliers, recovered=int(recovered),
                parallax=parallax, rotation_01=R01.tolist(), translation_01=t10.tolist())
            if (inliers < max(16, 0.35 * len(points1)) or
                    recovered < max(8, 0.15 * len(points1)) or
                    not np.isfinite(R01).all() or not np.isfinite(t10).all() or
                    not np.isfinite(parallax) or parallax < 1e-4):
                raise RuntimeError(
                    "OpenCV pose unreliable: matches={} inliers={} recovered={} "
                    "parallax={:.6f}rad".format(len(points1), inliers, recovered, parallax))
        Es[i] = torch.from_numpy(E).float()
        rot[i] = torch.from_numpy(cv2.Rodrigues(R01)[0][:, 0]).float()
        trans[i] = torch.from_numpy(t10).float()
